\documentclass[11pt]{template/cauchy}
\usepackage[czech]{babel}

% IMPORTY
\usepackage{enumerate}
\usepackage{mlmodern}
\usepackage{template/mathsphystools}
\usepackage{amsthm,thmtools,xcolor,amsmath,amssymb, mathtools}
\usepackage[scr]{rsfso}
% \usepackage{thmstyles}
\usepackage{tabularx}
\usepackage{graphicx}
\usepackage{multicol}
\usepackage{appendix}
\usepackage{tabularx}
\usepackage{subcaption}
\usepackage{hyperref}
\usepackage{comment}
\usepackage{lscape}
\usepackage{xcolor}
\usepackage{wrapfig}
\usepackage{centernot}
\usepackage{pgfplots}
\usepackage{caption}
\usepackage{cancel}
\usepackage{marvosym}
\usepackage{etoolbox}

\makeatletter
\patchcmd{\l@section}{\hfil}{%
  \leaders\hbox{$\m@th
    \mkern \@dotsep mu\hbox{.}\mkern \@dotsep
  mu$}\hfill}{}{}
\makeatother
\graphicspath{{images/}}
\pgfplotsset{width=10cm,compat=1.9}

% \usepackage{tikzit}
% \input{default.tikzstyles}
% je potřeba přiložit soubor default.tikzstyles


% STYLY
% definice, příklad, cvičení, věta, lemma, řešení, poznámka, důsledek, axiom, motivace

\declaretheoremstyle[
  headfont=\color{red}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{def}

\declaretheoremstyle[
  headfont=\color{blue}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{pr}

\declaretheoremstyle[
  headfont=\color{teal}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont\itshape,
]{cv}

\declaretheoremstyle[
  headfont=\color{green}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont\itshape,
]{veta}

\declaretheoremstyle[
  headfont=\color{lime}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont\itshape,
]{lemma}

\declaretheoremstyle[
  headfont=\color{black}\normalfont\itshape,
  bodyfont=\color{black}\normalfont,
  numbered=no
]{res}

\declaretheoremstyle[
  headfont=\color{brown}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{pozn}

\declaretheoremstyle[
  headfont=\color{brown}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{dusl}

\declaretheoremstyle[
  headfont=\color{cyan}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{axiom}

\declaretheoremstyle[
  headfont=\color{black}\normalfont\bfseries,
  bodyfont=\color{black}\normalfont,
]{motivace}

\declaretheorem[
  style=def,
  name=Definice,
  unnumbered,
]{definice}

\declaretheorem[
  style=pr,
  name=Příklad,
  unnumbered,
]{priklad}

\declaretheorem[
  style=cv,
  name=Cvičení,
  unnumbered,
]{cviceni}

\declaretheorem[
  style=res,
  name=Řešení,
  unnumbered,
]{reseni}

\declaretheorem[
  style=veta,
  name=Věta,
  numberwithin=section,
]{veta}

\declaretheorem[
  style=lemma,
  name=Lemma,
  numberwithin=section
]{lemma}

\declaretheorem[
  style=pozn,
  name=Poznámka,
  unnumbered,
]{poznamka}

\declaretheorem[
  style=dusl,
  name=Důsledek,
  unnumbered,
]{dusledek}

\declaretheorem[
  style=axiom,
  name=Axiom,
  numberwithin=section,
]{axiom}

\declaretheorem[
  style=motivace,
  name=Motivace,
  unnumbered,
]{motivace}

\DeclareMathOperator{\tg}{tg}
\DeclareMathOperator{\cotg}{cotg}
\DeclareMathOperator{\arctg}{arctg}
\DeclareMathOperator{\arccotg}{arccotg}
\DeclareMathOperator{\sgn}{sgn}

\title{\Large Matematická analýza I-II \\ {\large Zápisky z přednášky doc. RNDr. Dalibora Pražáka, Ph.D.}}
\author{Jan Romanovský}
\date{verze z \today}

\begin{document}

\maketitle
\frontmatter
\newpage
\tableofcontents
% \listoffigures
% \listoftables
\clearpage

% ÚVODNÍ INFORMACE
% dobré pro poznačení informačních věcí od učitele,
% např. jeho email, jména skript apod.
\input{additional/misc_info.tex}

% ZNAČENÍ
% pokud nepotřebujete, zakomentujte řádek níže
% všechny změny provádějte v souboru additional/conventions.tex
\input{additional/conventions.tex}
\clearpage

\mainmatter

% KAPITOLY
% \input{chapters/01_nazev_kapitoly}

%\input{chapters/00_nevim}
\input{chapters/01_uvod}
\input{chapters/02_limita_funkce}
\input{chapters/03_}
\input{chapters/04_}
\input{chapters/05_}
\input{chapters/06_hlubsi_vlastnosti}
\input{chapters/07_posloupnosti}
\input{chapters/08_taylor}
\input{chapters/09_urcity_integral}
\input{chapters/X_ciselne_obory}
\input{chapters/10_rady}
\input{chapters/11_mocninne_rady}
\input{chapters/12_dif_rce}
\input{chapters/13_metricke_prostory}
\input{chapters/14_fce_vice_promennych}

% \listoffigures

\backmatter

\end{document}
