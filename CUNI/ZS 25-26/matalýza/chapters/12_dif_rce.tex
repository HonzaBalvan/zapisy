...
Řešením rovnice (\ref{1}) rozumíme funkci $y(x): I \to \R$, která má vlastní derivace až do řádu $n$ všude v $I$, a
\[
...
F(x, y, y') = 0,
\end{equation}
kde $F = F(x, y, z)$ je nekonstantní vůči $z$. Řešením rovnice (\ref{2}) je funkce $y(x): I \to \R$, přičemž $y'(x)$ existuje vlastní všude v $I$ a $F(x, y(x), y'(x)) = 0$ pro $\forall x \in I$.
\end{definice}
...
Je dána rovnice (\ref{3}). Nechť $a(x)$, $b(x)$ jsou spojité v $I$, nechť $A(x) = \int a(x) \d x$, $B(x) = \int b(x) \exp A(x) \d x$ v $I$. Nechť $c \in \R$ je libovolné. Potom
\[
y(x) = \exp(-A(x))[B(x) + c]
\]
je řešení rovnice (\ref{3}) v $I$. Naopak: všechna řešení rovnice (\ref{3}) mají tento tvar.
\end{veta}
...
Je dána rovnice (\ref{4}). Nechť $f(x)$ je spojitá v $I$, nechť $g(y)$ je spojitá a nenulová v $J$. Nechť $F(x)$ resp. $G(y)$ jsou p.f. k $f(x)$ resp. $1/g(y)$ v $I$ resp. v $J$.
...
y' = f(x, y),
\end{equation}
- tzv. \textbf{rovnice vyřešená vzhledem k derivaci} – která je z hlediska teorie „šikovnější“ než obecný tvar (\ref{2}).
\end{poznamka}

\begin{lemma}[O napojování]
Nechť $y_1(x): (a, x_0) \to \R$, $y_2(x): (x_0, b) \to \R$ jsou řešení rovnice (\ref{5}). Nechť $\lim_{x \to x_0^-} y_1(x) = y_0 = \lim_{x \to x_0^+} y_2(x)$. Nechť $f(x, y)$ je spojitá v bodě $(x_0, y_0) \in \R^2$. Potom funkce
\[
y(x) = \begin{cases}
y_1(x), & x \in (a, x_0) \\
y_2(x), & x \in (x_0, b) \\
...
\end{cases}
\]
je řešením rovnice (\ref{5}) v celém $(a, b)$.
\end{lemma}
...
\begin{poznamka}
Obecný postup řešení rovnice se separovanými proměnnými (\ref{4}):
\begin{itemize}
...
\begin{definice}
Řekneme, že řešení $y(x): I \to \R$ rovnice (\ref{5}) \textbf{prochází bodem} $(x_0, y_0) \in \R^2$, jestliže (i) $x_0 \in I$ a (ii) $y(x_0) = y_0$.
...
\begin{poznamka}
Jestliže $y(x)$ řešení rovnice (\ref{5}) prochází bodem $(x_0, y_0)$, pak nutně $y'(x_0) = f(x_0, y(x_0)) = f(x_0, y_0)$. Geometricky: pokud stejným bodem prochází více řešení, mají zde stejnou tečnu – jejich grafy se „dotýkají“, nemohou se „křížit“.
\end{poznamka}

\begin{veta}[Peano]
Nechť $f(x, y)$ je spojitá na okolí bodu $(x_0, y_0)$. Potom bodem $(x_0, y_0)$ prochází alespoň jedno řešení rovnice (\ref{5}). Podrobněji: existuje $\delta > 0$ a funkce $y(x): (x_0 - \delta, x_0 + \delta) \to \R$, která řeší (\ref{5}) a splňuje $y(x_0) = y_0$.
\end{veta}

\begin{veta}[Picard]
Nechť funkce $f(x, y)$, $\frac{\partial}{\partial y} f(x, y)$ jsou spojité na okolí bodu $(x_0, y_0)$. Potom bodem $(x_0, y_0)$ prochází právě jedno řešení rovnice (\ref{5}).

Podrobněji: existuje $y(x)$ řešení (\ref{5}) procházející $(x_0, y_0)$, a je-li $\tilde{y}(x)$ jiné řešení, procházející $(x_0, y_0)$, tak existuje $\delta > 0$ tak, že $y(x) = \tilde{y}(x)$ pro $\forall x \in U(x_0, \delta)$.
...
Řešení $y(x): (a, b) \to \R$ rovnice (\ref{5}) určitě nejde prodloužit za bod $x = b$, jestliže (i) $b = +\infty$ nebo (ii) $y(x) \to \infty$ pro $x \to b-$ a nebo (iii) $y(x) \to y_0$ pro $x \to b-$, avšak bod $(b, y_0)$ neleží v definičním oboru funkce $f(x, y)$.

Jak poznám, že mám všechna řešení? Nechť $\Omega \subset \R^2$ je otevřená a funkce $f(x, y)$, $\frac{\partial}{\partial y} f(x, y)$ jsou spojité v $\Omega$. Jestliže najdu nějakou třídu $\{y_c(x)\}_{c \in \R}$ řešení rovnice (\ref{5}) takových, že jejich grafy vyplní celou $\Omega$, pak díky Větě 12.4 jsou to zároveň všechna řešení, která se v $\Omega$ mohou vyskytnout.
\end{poznamka}
...
\boldsymbol{y}(x_0) = \boldsymbol{\eta},
\end{equation}
...
\begin{poznamka}[d'Alembertova transformace] Funkce $y(x): I \to \R$ je řešením rovnice (\ref{8}), právě když vektorová funkce $z(x): I \to \R^n$, kde
\[
\begin{array}{l}
z_1(x) = y(x) \\
z_2(x) = y'(x) \\
...
Řešením rovnice (\ref{9}) rozumíme funkci $y(x) \in C^n(I)$, která splňuje (\ref{9}) pro $\forall x \in I$.

\textbf{Klíčový předpoklad.}\label{P} O rovnici (\ref{9}) budeme předpokládat, že $a_i(x)$, $i = 1, \ldots, n$ a $f(x)$ jsou spojité v $I$ (otevřený interval), navíc $a_0(x) \neq 0$ pro $\forall x \in I$. \quad (P)

\begin{veta}
Je dána rovnice (\ref{9}) a platí předpoklad (\ref{P}). Nechť $x_0 \in I$ a $\eta \in \R^n$ jsou libovolné. Potom existuje jediná funkce $y(x) \in C^n(I)$, která řeší (\ref{9}) na celém $I$, a splňuje počáteční podmínky
\[
\begin{aligned}
y(x_0) &= \eta_1 \\
y'(x_0) &= \eta_2 \\
...
\textbf{Značení.} Při značení $\mathcal L [y] = \sum_{k=0}^n a_k(x) y^{(n-k)}$ můžeme rovnici (\ref{9}) přepsat jako $\mathcal L [y] = f$. Speciální případ $f = 0$ je tzv. homogenní úloha
\begin{equation}\label{10}
\mathcal L [y] = 0.
\end{equation}

\begin{veta}[Struktura řešení lineární rovnice řádu $n$]
Nechť platí předpoklad (\ref{P}). Potom:
\begin{enumerate}
    \item Množina $\mathcal {H}$ všech řešení homogenní úlohy (\ref{10}) tvoří lineární podprostor dimenze $n$ v prostoru $C^n(I)$.
    \item Množina $\mathcal {N}_f$ všech řešení úlohy (\ref{9}) má tvar $\{y_p + y; y \in \mathcal {H}\}$, kde $y_p$ je libovolné, pevně zvolené řešení této úlohy.
...
\textbf{Terminologie.} Bází prostoru $\mathcal {H}$ nazýváme \textbf{fundamentálním systémem} (F.S.) rovnice (\ref{9}). Pevně zvolené řešení $y_p$ nazýváme \textbf{partikulární řešení}.
...
Je dána rovnice (\ref{9}) $\mathcal L [y] = f$ a platí (\ref{P}). Nechť $\{y_1, \ldots, y_n\}$ je fundamentální systém. Nechť $c_1'(x), \ldots, c_n'(x)$ splňují pro $\forall x \in I$ soustavu
\[
\begin{aligned}
...
\end{aligned}
\]
Potom funkce $y(x) = \sum_{j=1}^n c_j(x) y_j(x)$ je řešením úlohy (\ref{9}).
\end{veta}
...
b_0 y^{(n)} + b_1 y^{(n-1)} + \cdots + b_n y = f(x),
\end{equation}
...
p(\lambda) = \sum_{k=0}^n b_k \lambda^{n-k}.
\end{equation}
Tedy: je-li $\lambda_0$ kořenem polynomu $p(\lambda)$, tak funkce $\exp(\lambda_0 x)$ je řešením homogenní úlohy.
\end{poznamka}

\begin{definice}
Polynom (\ref{13}) se nazývá \textbf{charakteristický polynom} rovnice (\ref{11}).
\end{definice}
...
\begin{dusledek}
Je-li $\lambda_0$ kořen char. polynomu násobnosti $k$, pak funkce $\exp(\lambda_0 x)$, $x \exp(\lambda_0 x)$, $\ldots$, $x^{k-1} \exp(\lambda_0 x)$ řeší homogenní úlohu (\ref{12}).
\end{dusledek}
...
\]
tvoří fundamentální systém homogenní úlohy (\ref{12}).
\end{veta}
