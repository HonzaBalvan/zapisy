...
\begin{priklad} $f(x) = \frac{1}{x}$ ... $D_f = \mathbb R \smallsetminus \{0\}$, nikoliv $\frac{1}{\pm \infty} = 0$8
\end{priklad}
...
\begin{definice}
  Funkce f(x) se nazve na množině M
  \begin{enumerate}[i.]
...
\begin{definice}[Okolí bodu]
  Nechť $\delta > 0, x \in \mathbb R$. Pak
  \begin{enumerate}[i.]
...
\begin{veta}[Princip oddělení]
  Nechť $x_0, x_1 \in \mathbb R^*, x_0 \neq x_1$ t. ž. $\mathscr U (x_0, \delta) \cap \mathscr (x_1, \delta) = \emptyset$.
\end{veta}

\begin{proof}.
  BÚNO $x_0<x_1$, rozlišme případy:
  \begin{enumerate}[i.]
...
\begin{poznamka}
  Dokázali jsme navíc, že $\mathscr U(x_0, \delta)$ je vlevo od $\mathscr U (x_1, \delta)$, tj. $\forall x \in \mathscr U(x_0, \delta), \forall y \in \mathscr U (x_1, \delta): x<y$.
\end{poznamka}

\begin{definice}
  Nechť $x_0 \in \mathbb R^*, f(x)$ je definována na jistém $\mathscr P(x_0)$. Číslo $A \in \mathbb R^*$ nazveme limitou $f(x)$ v bodě $x_0$, jestliže:
  $$\forall \epsilon >0 \exists \delta >0: x \in \mathscr P(x_0, \delta) \implies f(x) \in \mathscr U (A, \epsilon)$$
...
\begin{priklad}
  Dirichletova funkce:
  $$D(x) \coloneq \begin{cases} 1, x \in \mathbb Q \\ 0, x \in \mathbb R \smallsetminus \mathbb Q \end{cases}$$
  tvrdíme: $\nexists lim_{x \to x_0}{D(x)}, x_0 \in \mathbb R^*$ lib.
\end{priklad}
