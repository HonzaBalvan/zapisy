dusledek\section{Mocninné řady}
...
\begin{dusledek}
Taky řady $\sum_{k=2}^{\infty}k(k-1)c_k(z - z_0)^{k-2}$, $\sum_{k=1}^{\infty}\frac{c_k}{k+1}(z - z_0)^{k+1}$ mají stejný poloměr konvergence jako řada $\sum_{k=0}^{\infty}c_k(z - z_0)^k$.
\end{dusledek}
...
\begin{priklad}
  ...
  \begin{itemize}
    ...
    \item $\phi^\prime (x) = \begin{cases}
      0, x<0 \\ \frac{1}{x^2}\cdot e^{-\frac{1}{x}}\end{cases}$\\
      $\phi^\prime_-(0)=0$\\
      $\phi^\prime_+=0$
