...
\begin{poznamka} Značení:
  \begin{itemize}
    ...
    \item $P \land Q$ \dots $P$ zároveň s $Q$
    ...
    \item $A \smallsetminus B$ \dots rozdíl množin (prvky z $A$, které nejsou v $B$)
  \end{itemize}
\end{poznamka}

\begin{axiom}[Algebraické vlastnosti $\mathbb{R}$]
  Existuje množina reálných čísel $\mathbb{R}$, která obsahuje prvky 0 a 1, a jsou na ní definovány operace \uv{$\cdot$} a \uv{$+$} tak, že $\forall x, y, z \in \mathbb{R}$ platí:
  \begin{enumerate}[i.]
    \item $x+y = y+x, x \cdot y = y \cdot x$
    \item $x+(y+z) = (x+y)+z, x\cdot (y\cdot z) = (x \cdot y) \cdot z$
    \item $x \cdot (y+z) = x \cdot y + x \cdot z$
    \item $0 + x = x$, $1 \cdot x = x$
    \item $0 \cdot x = 0$ a naopak $x \cdot y = 0 \iff x = 0 \lor y = 0$
  \end{enumerate}
\end{axiom}

\begin{axiom}[Uspořádání $\mathbb{R}$]
  Na množině $\mathbb{R}$ je definována relace \uv{$<$} tak, že $\forall x, y, z \in \mathbb{R}$ platí:
  \begin{enumerate}[i.]
    \item $x = y$, nebo $x < y$, nebo $y < x$
    \item $x < y \land y < z \implies x < z$
    \item $x < y \implies x + z < y + z$
    \item $0 < x \land 0 < y \implies 0 < x \cdot y$
  \end{enumerate}
\end{axiom}
...
\begin{poznamka}[Význačné podmnožiny $\mathbb R$]
  ...
  \begin{itemize}
    ...
    \item celá čísla $\mathbb Z = \mathbb N \cup \{0, -1, -2, \dots\}$
...
\begin{definice}
  Nechť $x \in \mathbb R$. Potom absolutní hodnota z x, značíme $|x| \coloneq \begin{cases}x, x \geq 0 \\ -x, x < 0\end{cases}$
\end{definice}
...
\begin{axiom}[B. Odmocnina v $\mathbb R$]
  \begin{itemize}
    \item Nechť  $n \in \mathbb N$ je sudé. Pak $\forall a \in [0, \infty) \exists! b \in [0, \infty)$ takové, že $b^n = a$
    \item Nechť $n \in \mathbb N$ je liché. Pak $\forall a \in \mathbb R \exists ! b \in \mathbb R$ takové, že $b^n = a (b = \root n \of {a})$.
  \end{itemize}
\end{axiom}

\begin{poznamka}
  \begin{itemize}
    \item $\sqrt{1} = 1, \root 3 \of {-1} = -1, \sqrt{-1}$ není definována
    \item $\root n \of {x^n} = x \forall x \in \mathbb R, n$ liché
    \item $\sqrt{x^2} = |x|$
  \end{itemize}
\end{poznamka}

\begin{veta}
  Existují iraconální čísla.
\end{veta}

\begin{axiom}[Axiomy $\mathbb N$]
  \begin{itemize}
    \item $\forall x \in \mathbb R \exists n \in \mathbb N: n > x$ (Archimédova vlastnost)
    \item Nechť $M \subseteq \mathbb N$ splňuje:
    \begin{enumerate}[i.]
      \item $1 \in M$
      \item $\forall n \in \mathbb N: n \in M \implies n+1 \in M$
    \end{enumerate} potom $M = \mathbb N$. (princip indukce)
  \end{itemize}
\end{axiom}

\begin{poznamka}[alternativní ekvivalentní formulace]
  \begin{itemize}
    \item $\forall \varepsilon > 0 \exists n \in \mathbb N: n\varepsilon > 1$
    \item Nechť $\phi (n)$ je formule s proměnnou $n \in \mathbb N$, nechť $n_0 \in \mathbb N$. Nechť platí:
    \begin{enumerate}[i.]
      \item $\phi(n)$
      \item $\forall n \geq n_0: \phi (n) \implies \phi (n+1)$
    \end{enumerate}
    potom platí $\phi (n) \forall n \geq n_0$.
  \end{itemize}
\end{poznamka}

\begin{poznamka}[indukce ještě jinak]
...
\begin{veta}
  Každý otevřený interval obsahuje nekonečně mnoho racionálních a iracionálních čísel.
\end{veta}

\begin{proof}[pro iracionální]
...
\begin{definice}
  Nechť $M \subset \mathbb R$, Číslo $S \in \mathbb R$ nazveme supremum $M$, značíme $S = \sup M$, jestliže:
  \begin{enumerate}[i.]
...
\begin{axiom}[Úplnost $\mathbb R$ = Axiom suprema]
  Nechť $M \subset \mathbb R$ je neprázdná a shora omezená. Pak $\exists! S \in \mathbb R$ takové, že $S = \sup M$.
\end{axiom}

\begin{definice}
  Nechť $M \subset \mathbb R$, Číslo $s \in \mathbb R$ nazveme infimum $M$, značíme $s = \inf M$, jestliže:
  \begin{enumerate}[i.]
...
\begin{axiom}[A4$^\prime$]
  Nechť $M \subset \mathbb R$ je neprázdná a zdola omezená. Pak $\exists! s \in \mathbb R$ takové, že $s = \inf M$.
\end{axiom}

\begin{definice}[Rozšířená reálná čísla]
  $\mathbb R^* = \mathbb R + \{- \infty, + \infty\}$. Uspořádání a aritmetika v $\mathbb R^*$:
  \begin{itemize}
...
\begin{definice}
  Nechť $X, Y$ množiny. Funkce $f: X \rightarrow Y$ je libovolný předpis, který každému $x \in \mathbb X$ přiřadí jednozačně určený prvek z $Y$. Dále definujme
  \begin{itemize}
    \item obraz množiny $M \subset X: f(M) = \{f(x); x \in M\}$
    \item vzor množiny $N \subset Y: f^{-1}(N) = \{x, f(x) \in N\}$ (zde \uv{$^-1$} není inverzní funkce, toto můžu říct i pro nezinvertovatelnou funkci)
  \end{itemize}
  Funkce je
  \begin{itemize}
    \item prostá (injektivní), pokud $x \neq y \implies f(x) \neq f(y)$
    \item "na" (surjektivní), pokud $f(X)=Y$, tj. $\forall y \in Y \exists x \in X: f(x)=y$
    \item je vzájemně jednoznačná (bijektivní), je-li prostá i "na"
  \end{itemize}
\end{definice}
