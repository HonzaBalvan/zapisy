...
\section{Hlubší vlastnosti derivace a spojitosti}

\begin{veta}[L'Hospital]
  Nechť $x_0 \in \mathbb R^*$, nechť $\exists$ vlastní $f^\prime (x), g^\prime (x)$, navíc $g^\prime (x) \neq 0 \,\forall x_0 \in \mathscr P(x_0, \delta)$. Nechť platí buď
\begin{enumerate}[a)]
  \item $f(x) \to 0, g(x) \to 0, x \to x_0$, nebo
  \item $|g(x)| \to +\infty, x \to x_0$.
\end{enumerate}
  Pak
...
\begin{proof}
  \begin{enumerate}[1.]
    \item $x \to x_0^+, x_0 \in \mathbb R$, případ a); $\varepsilon > 0$ dáno:

          $\exists \delta > 0$ t. ž. $\forall c \in \mathscr P_+(x_0, \delta)$ platí:
          $$\frac{f^\prime(x)}{g^\prime(x)} \in \mathscr U(L, \varepsilon)$$
          ...
          $\implies \exists x \in [x_0, x]\subset \mathscr P_+(x_0, \delta)$ t. ž. $\frac{f(x)}{g(x)}=\frac{f(x_0)-f(x)}{g(x_0)-g(x)} = \frac{f^\prime (x)}{g^\prime (x)} \in \mathscr U (L, \varepsilon) $
...
\end{veta}
\begin{proof}
  Nechť $x_1 < x_2\in I \stackrel{?}{\implies} f(x_1)<f(x_2)$.
...
\begin{proof}
  Nechť $x_1 < x_2 < x_3 \in I \stackrel{?}{\implies} \frac{f(x_2)-f(x_1)}{x_2-x_1} < \frac{f(x_3)-f(x_2)}{x_3-x_2}$.
...
\begin{proof}
  Důkaz jako cvičení pro čtenáře, napovíme $x_1 = a, x_3 = b, x_2 = $ nějakej zlomek s $\lambda$
\end{proof}

\begin{veta}[Konvexita a znaménko druhé derivace]
  Nechť $f^\prime x$ je spojitá v $I$, nechť $\exists f^{\prime\prime}(x)$ vlastní uvnitř $I$. Nechť platí $f^{\prime\prime}(x)>0$ (resp. $\geq, < \leq$) $\forall x \in I$ vnitřní. Pak $f(x)$ je v $I$ ryze konvexní (resp. konvexní, ryze konkávní, konkávní).
\end{veta}

\begin{proof}
  Označ $\tilde{I}$ vnitřek $I$.

  $f^{\prime\prime}(x) = (f^\prime(x))^\prime >0$ v $\tilde{I}$, konečné

  $\implies f^\prime(x)$ spojité (V. 5. 1.) a je rostoucí v $\tilde{I}$ (V. 6. 10.)

  $\implies f(x)$ je ryze konvexní v $I$ (V. 6. 11.).
\end{proof}

\begin{definice}
  Řekneme, že $x_0$ je inflexní bod $f(x)$, jestliže
  \begin{enumerate}[i.]
    \item $\exists f^\prime(x_0) \in \mathbb R^*$
    \item $\exists \delta >0$ t. ž. na jednom z intervalů $(x_0-\delta, x_0), (x_0, x_0 + \delta)$ je $f(x)$ ryze konvexní a na druhé ryze konkávní.
  \end{enumerate}
\end{definice}
