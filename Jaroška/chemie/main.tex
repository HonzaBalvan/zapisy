...
typy vzorců
\begin{itemize}
  \item stechiometrický -- \ce{NH2}, \ce{H2NO}
  \item funkční -- \ce{NH2}, \ce{NH4NO2}
  \item molekulový -- \ce{NO2} (funkční) vs. \ce{N2O4} (molekulový)
  \item další
\end{itemize}
...
\begin{itemize}
  ...
  \item nebo hydridy s vodíkem -I, názvosloví: část mezin. jména prvku a koncovka -an, např. \ce{AlH3} -- hydrid hlinitý, ale i \textbf{alan}, \ce{BH3} -- boran, \ce{SiH4} -- silan, \ce{PH4} -- fosfan, \ce{AsH3} -- arsan, \ce{H2S} -- sulfan, \ce{B2H6} -- diboran, \ce{Si2H6} -- disilan, \ce{P2H4} -- difosfan, \ce{SbH3} -- stiban, \ce{BiH3} -- bismutan
\end{itemize}

\subsubsection{Kationty}
\begin{itemize}
  \item jednoatomové láčes deczi
  \item ty víceatomové přijímají koncovku -onium, např. \ce{PH^{4+}} -- fosfonium, \ce{AsH^{4+}} -- arsonium, \ce{SbH^{4+}} -- stibonium, \ce{OH^{3+}} -- oxonium, \ce{SH^{3+}} -- sulfonium, \ce{SeH^{3+}} -- selenonium, \ce{TeH^{3+}} -- telluronium , \ce{PH^{2+}} -- fluoronium, \ce{IH^{2+}} -- jodonium
  \item když mám kyselé H navíc: \ce{CH3COOH2^{+}} -- acetacidium
  \item \ce{NH4^{+}} -- amonný kation, amonium
  \item \ce{H3O^{+}} -- oxonium/hydroxonium/hydronium
\end{itemize}
...
Když s tím chci počítat -- \uv{náhodou} je počet gramů látky na mol stejný jako jeho relativní molekulová hmotnost (avogadrova konstanta vybraná/definována tak, aby to vycházelo). Potom $n = \frac{m}{M_R}$ pokud $m$ je v gramech. Dále $A_r = \frac{m}{m_u}$ (vychází z předch. vzorců).\\
...
\ce{NaF + Al2 (SO4)3 -> Na2SO4^{+} + Na3[AlF6]}
Před sloučeniny si dám proměnné a dopočítám jako soustavu rovnic pro jednotlivé prvky: \ce{\textit{a}NaF+\textit{b}Al2(SO4)3 -> \textit{c}Na2SO4 + \textit{d}Na3[AlF6]}
\begin{align*}
  \text{Na: } &1a = 2c + 3d\\
  \text{F: } &1a = 6d\\
  \text{Al: } &2b = 1d\\
...
\begin{itemize}
    ...
    \item spinové číslo -- s -- \pm \frac{1}{2} -- určuje spin elektronu
\end{itemize}
\begin{itemize}
...
\begin{itemize}
    ...
    \item když disociační reakce probíhá tam i zpátky můžu identifikovat tzv. konjugovanou bázi, tj. dva stavy \uv{jedné} molekuly na dvou stranách rovnice, třeba $CH_3COOH$ a $CH_3COO^-$ nebo $H_20$ a $H_3O^+$, ale také $H_2O$ a $OH^-$
...
$$[X^{m+}] = n \cdot s$$
$$[Y^{m-}] = m \cdot s$$
, kde $s$ je rozpustnost = látková koncentrace

\subsubsection{Součin rozpustnosti}
$$K_S = K \cdot [X_mY_n] = [X+]^m \cdot [Y-]^n}$$
, kde $K$ je rovnovážná konstanta zpětné reakce $X_mY_n < - > mX^+ + nY^-; K = \frac{[X^+]^m \cdot [Y^-]^n}{[X_mY_n]}$.

$$\implies K_S = [n \cdot s]^m [m \cdot s]^n$$
\end{document}
