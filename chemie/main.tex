...
\begin{itemize}
  ...
  \item molekulový -- \ce{NO2}(f) vs. \ce{N2O4}(m)
  \item další
\end{itemize}
...
\begin{itemize}
  ...
  \item \ce{NH4^{+}} -- amonny kation, amonium
...
Když s tím chci počítat -- \uv{náhodou} je počet gramů látky na mol stejný jako jeho relativní molekulová hmotnost. Potom $n = \frac{m}{M_R}$ pokud $m$ je v gramech. Dále $A_r = \frac{m}{m_u}$ (vychází z předch. vzorců).\\
...
\ce{Cr2^{III}O3^{-II} + K^{I}N^{V}O3^{-II} + K^{I}O^{-II}H^{I} -> K3CrO4 + KNO2 + H2O}, takže změny u $Cr, N$
oxidace: \ce{2Cr^{III}->^{6e}2Cr^{VI}} - - křížové pravidlo 2 -> 1
redukce: \ce{N^{V}->^{2e}N^{III}} - - křížové pravidlo 6 -> 3
\ce{\textit{1}Cr2O3 + \textit{3}KNO3 + 4KOH -> 2K_3CrO4 + 3KNO2 + 2H2O}

\subsubsection{V iontové formě}
\ce{IO3^{-}+Cl2 + OH^{-} -> IO6^{5-} + Cl^{-}+ H2O}

\subsubsection{Disproporcionační rovnice}
\ce{MnO4^{2-}+ H3O^{+} -> MnO4^{-} + MnO2 + H2O}
\ce{H2S + SO2 -> S + H2O}
\end{document}
